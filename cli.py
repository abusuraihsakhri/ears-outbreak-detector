"""
Command Line Interface for Ears Outbreak Detector.
"""
import argparse
import csv
import json
...
def main(argv=None):
    parser = argparse.ArgumentParser(prog="ears-outbreak-detector", description="Ears Outbreak Detector")
    ...
    if args.command == "audit":
        ...
        print("=" * 80)
        print(f"  EARS OUTBREAK DETECTOR")
    ...
    if args.command == "chat":
        ...
        print(f"\n[Ears Outbreak Detector Supervisor]:\n{ans}\n")
        return 0
    ...
    if args.command == "batch":
        ...
        out_rows = []
        errors = 0
        for idx, r in enumerate(rows, start=2):  # start=2: row 1 is header
            try:
                primary_raw = r.get("primary_metric", "15.0")
                secondary_raw = r.get("secondary_metric", "5.0")
                payload = SystemTaskPayload(
                    task_id=r.get("task_id", "TASK-01"),
                    target_identifier=r.get("target_identifier", "TARGET-01"),
                    primary_metric=float(primary_raw) if primary_raw else 15.0,
                    secondary_metric=float(secondary_raw) if secondary_raw else 5.0,
    ...
    if args.command == "serve":
        ...
        print(f"Starting Ears Outbreak Detector API server on http://{args.host}:{args.port}")
